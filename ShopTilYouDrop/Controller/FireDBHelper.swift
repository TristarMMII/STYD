...
class FireDBHelper: ObservableObject {
    ...
    func setUserData(userData: User){
        ...
        data.updateData(["Wishlist" : userData.wishList, "History": userData.history]) { error in
            if let error = error {
    ...
    func addHistoryData(product: Product.Datum){
        let db = Firestore.firestore()
        let data = db.collection("UserData").document(Auth.auth().currentUser!.uid)
        ...
        data.updateData(["History": FieldValue.arrayUnion([productData])]) { error in
            if let error = error {
                print(error.localizedDescription)
            }
        }
